...
/*   Updated: 2021/06/30 13:59:25 by ssamadi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
long long curr()
{
	struct timeval time;
	gettimeofday(&time, NULL);
	long long h = (time.tv_usec / 1000) + (time.tv_sec * 1000);
	return (h);
}

// int cond(t_philo_data *ph)
// {
// 	t_all *all;

// 	all = all_t();
// 	if (get_in_mic() - ph[j]->last_eat >= all->time_to_die * 1000)
// 		ph->is_dead = 1;
// 	return (1);
// }
